...
bool CTwBar::MouseMotion(int X, int Y)
{
    ...
    if( !m_IsMinimized )
    {
        bool InBar = (X>=m_PosX && X<m_PosX+m_Width && Y>=m_PosY && Y<m_PosY+m_Height);
        ...
        {   
            ...
            m_HighlightDecrBtn = false;
            m_HighlightRotoBtn = false;
            if( abs(m_MouseOriginX-X)>6 || abs(m_MouseOriginY-Y)>6 )
                m_HighlightClickBtn = false;
            m_HighlightListBtn = false;
            m_HighlightTitle = false;
            m_HighlightScroll = false;
            ...
            m_HighlightLabelsHeader = false;
            m_HighlightValuesHeader = false;
            //if( InBar && X>m_PosX+m_Font->m_CharHeight+1 && X<m_PosX+m_VarX2 && Y>=m_PosY+m_VarY0 && Y<m_PosY+m_VarY1 )
            if( InBar && X>m_PosX+2 && X<m_PosX+m_VarX2 && Y>=m_PosY+m_VarY0 && Y<m_PosY+m_VarY1 )
            {   // mouse over var line
                m_HighlightedLine = (Y-m_PosY-m_VarY0)/(m_Font->m_CharHeight+m_LineSep);
                ...
                else
                {
                    ...
                    else if( m_DrawRotoBtn && ( X>=m_PosX+m_VarX2-IncrBtnWidth(m_Font->m_CharHeight) || X<m_PosX+m_VarX1 ) )  // [o] button
                    //else if( m_DrawRotoBtn && X<m_PosX+m_VarX1 ) // [o] button
                    {
                        m_HighlightRotoBtn = true;
                        ANT_SET_CURSOR(Point);
                    }
                    else if( m_DrawIncrDecrBtn && X>=m_PosX+m_VarX2-2*IncrBtnWidth(m_Font->m_CharHeight) ) // [+] button
                    {
                        m_HighlightIncrBtn = true;
                        ANT_SET_CURSOR(Arrow);
                    }
                    else if( m_DrawIncrDecrBtn && X>=m_PosX+m_VarX2-3*IncrBtnWidth(m_Font->m_CharHeight) ) // [-] button
                    {
            ...
            else if( InBar && m_Movable && !m_IsPopupList && X>=m_PosX+2*m_Font->m_CharHeight && X<m_PosX+m_Width-2*m_Font->m_CharHeight && Y<m_PosY+m_Font->m_CharHeight )
            {   // mouse over title
                m_HighlightTitle = true;
                ANT_SET_CURSOR(Move);
            }
            else if ( InBar && !m_IsPopupList && X>=m_PosX+m_VarX1-5 && X<m_PosX+m_VarX1+5 && Y>m_PosY+m_Font->m_CharHeight && Y<m_PosY+m_VarY0 )
            {   // mouse over ValuesWidth handle
                m_HighlightValWidth = true;
                ANT_SET_CURSOR(WE);
            }
            else if ( InBar && !m_IsPopupList && !m_IsHelpBar && X>=m_PosX+m_VarX0 && X<m_PosX+m_VarX1-5 && Y>m_PosY+m_Font->m_CharHeight && Y<m_PosY+m_VarY0 )
            {   // mouse over left column header
                m_HighlightLabelsHeader = true;
                ANT_SET_CURSOR(Arrow);
            }
            else if ( InBar && !m_IsPopupList && X>=m_PosX+m_VarX1+5 && X<m_PosX+m_VarX2 && Y>m_PosY+m_Font->m_CharHeight && Y<m_PosY+m_VarY0 )
            {   // mouse over right column header
                m_HighlightValuesHeader = true;
                ANT_SET_CURSOR(Arrow);
            }
            //else if( InBar && m_NbDisplayedLines<m_NbHierLines && X>=m_PosX && X<m_PosX+m_Font->m_CharHeight && Y>=m_ScrollY0 && Y<m_ScrollY1 )
            ...
            else if( InBar && X>=m_PosX+m_VarX2+2 && X<m_PosX+m_Width-2 && Y>=m_PosY+m_VarY0 && Y<m_ScrollY0 )
            {
                m_HighlightUpScroll = true;
                ANT_SET_CURSOR(Arrow);
            }
            else if( InBar && X>=m_PosX+m_VarX2+2 && X<m_PosX+m_Width-2 && Y>=m_ScrollY1 && Y<m_PosY+m_VarY1 )
            {
                m_HighlightDnScroll = true;
                ANT_SET_CURSOR(Arrow);
            }
            else if( InBar && m_Resizable && !m_IsPopupList && X>=m_PosX && X<m_PosX+m_Font->m_CharHeight && Y>=m_PosY && Y<m_PosY+m_Font->m_CharHeight )
                ANT_SET_CURSOR(TopLeft);
            else if( InBar && !m_IsPopupList && X>=m_PosX && X<m_PosX+m_Font->m_CharHeight && Y>=m_PosY+m_Height-m_Font->m_CharHeight && Y<m_PosY+m_Height )
                ANT_SET_CURSOR(BottomLeft);
            else if( InBar && m_Resizable && !m_IsPopupList && X>=m_PosX+m_Width-m_Font->m_CharHeight && X<m_PosX+m_Width && Y>=m_PosY && Y<m_PosY+m_Font->m_CharHeight )
                ANT_SET_CURSOR(TopRight);
            else if( InBar && m_Resizable && !m_IsPopupList && X>=m_PosX+m_Width-m_Font->m_CharHeight && X<m_PosX+m_Width && Y>=m_PosY+m_Height-m_Font->m_CharHeight && Y<m_PosY+m_Height )
                ANT_SET_CURSOR(BottomRight);
            else if( InBar && g_TwMgr->m_FontResizable && !m_IsPopupList && X>=m_PosX+m_Font->m_CharHeight && X<m_PosX+2*m_Font->m_CharHeight && Y<m_PosY+m_Font->m_CharHeight )
            {
                m_HighlightFont = true;
                ANT_SET_CURSOR(Arrow);
            }
            else if( InBar && m_Iconifiable && !m_IsPopupList && X>=m_PosX+m_Width-2*m_Font->m_CharHeight && X<m_PosX+m_Width-m_Font->m_CharHeight && Y<m_PosY+m_Font->m_CharHeight )
            {
                m_HighlightMinimize = true;
                ANT_SET_CURSOR(Arrow);
            }
            else if( m_IsHelpBar && InBar && X>=m_PosX+m_VarX0 && X<m_PosX+m_Width-m_Font->m_CharHeight && Y>m_PosY+m_Height-m_Font->m_CharHeight && Y<m_PosY+m_Height )
        ...
        else
        {
            ...
            {
                int y = m_PosY;
                m_PosX += X-m_MouseOriginX;
                m_PosY += Y-m_MouseOriginY;
                m_MouseOriginX = X;
                m_MouseOriginY = Y;
                int vpx, vpy, vpw, vph;
                vpx = 0;
            ...
            {
                m_ValuesWidth += m_MouseOriginX-X;
                m_MouseOriginX = X;
                NotUpToDate();
                if( m_IsHelpBar )
                    g_TwMgr->m_HelpBarNotUpToDate = true;
            ...
            else if( m_MouseDragScroll )
            {
                if( m_ScrollYH>0 )
                {
                    int dl = ((Y-m_MouseOriginY)*m_NbHierLines)/m_ScrollYH;
            ...
            else if( m_MouseDragResizeUL )
            {
                ...
                m_PosX += X-m_MouseOriginX;
                m_PosY += Y-m_MouseOriginY;
                m_Width -= X-m_MouseOriginX;
                m_Height -= Y-m_MouseOriginY;
                m_MouseOriginX = X;
                m_MouseOriginY = Y;
    ...
    {
        if( m_Iconifiable && X>=m_MinPosX+2 && X<m_MinPosX+m_Font->m_CharHeight && Y>m_MinPosY && Y<m_MinPosY+m_Font->m_CharHeight-2 )
        {
    ...
    if( g_TwMgr!=NULL && (!Handled || CustomArea) && !m_IsMinimized && m_CustomRecords.size()>0 )
    {
        ...
        for( int s=0; s<2; ++s )    // 2 iterations: first for custom widget having focus, second for others if no focused widget.
            for( CustomMap::iterator it=m_CustomRecords.begin(); it!=m_CustomRecords.end(); ++it )
            {
                ...
                {
                    if( sProxy->m_CustomCaptureFocus || (X>=r.m_XMin && X<r.m_XMax && Y>=r.m_YMin && Y<r.m_YMax) )
                    {
                        CustomHandled = sProxy->m_CustomMouseMotionCallback(X-r.m_XMin, Y-r.m_Y0, r.m_XMax-r.m_XMin, r.m_Y1-r.m_Y0, sProxy->m_StructExtData, sProxy->m_StructClientData, this, r.m_Var);
...
bool CTwBar::MouseButton(ETwMouseButtonID _Button, bool _Pressed, int X, int Y)
{
    assert(g_TwMgr->m_Graph && g_TwMgr->m_WndHeight>0 && g_TwMgr->m_WndWidth>0);
    bool Handled = false;
    if( !m_UpToDate )
    ...
    if( !m_IsMinimized )
    {
        Handled = (X>=m_PosX && X<m_PosX+m_Width && Y>=m_PosY && Y<m_PosY+m_Height);
        if( _Button==TW_MOUSE_LEFT && m_HighlightedLine>=0 && m_HighlightedLine<(int)m_HierTags.size() && m_HierTags[m_HighlightedLine].m_Var )
        {
            bool OnFocus = (m_HighlightedLine==(Y-m_PosY-m_VarY0)/(m_Font->m_CharHeight+m_LineSep) && Handled);
            if( m_HierTags[m_HighlightedLine].m_Var->IsGroup() )
            {
            ...
            else if( _Pressed && !m_MouseDrag )
            {
                ...
                {
                    // begin rotoslider
                    if( X>m_PosX+m_VarX1 && OnFocus )
                        RotoOnLButtonDown(m_PosX+m_VarX2-(1*IncrBtnWidth(m_Font->m_CharHeight))/2, Y);
                    else
                        RotoOnLButtonDown(X, Y);
                    m_MouseDrag = true;
                    m_MouseDragVar = true;
                }
                         ...
                         || (!Var->m_ReadOnly && EditInPlaceAcceptVar(Var)) )
                    {
                        ...
                        if( !m_EditInPlace.m_Active || m_EditInPlace.m_Var!=Var )
                        {
                            ...
                            if( EditInPlaceIsReadOnly() )
                                EditInPlaceMouseMove(X, Y, false);
                            m_MouseDrag = false;
                            m_MouseDragVar = false;
                        }
                        else
            ...
            else if( !_Pressed )
            {
                ...
                CustomArea = true;
                 ...
                 && ( (_Button==TW_MOUSE_LEFT && X>=m_PosX+2*m_Font->m_CharHeight && X<m_PosX+m_Width-2*m_Font->m_CharHeight && Y>=m_PosY && Y<m_PosY+m_Font->m_CharHeight)
                      || (_Button==TW_MOUSE_MIDDLE && X>=m_PosX && X<m_PosX+m_Width && Y>=m_PosY && Y<m_PosY+m_Height) ) )
        {
            m_MouseDrag = true;
        ...
        else if( _Pressed && !m_MouseDrag && !m_IsPopupList && _Button==TW_MOUSE_LEFT && X>=m_PosX+m_VarX1-3 && X<m_PosX+m_VarX1+3 && Y>m_PosY+m_Font->m_CharHeight && Y<m_PosY+m_VarY0 )
        {
            m_MouseDrag = true;
        ...
        else if( _Pressed && !m_MouseDrag && m_NbDisplayedLines<m_NbHierLines && _Button==TW_MOUSE_LEFT && X>=m_PosX+m_VarX2+2 && X<m_PosX+m_Width-2 && Y>=m_ScrollY0 && Y<m_ScrollY1 )
        {
            m_MouseDrag = true;
        ...
        else if( _Pressed && _Button==TW_MOUSE_LEFT && X>=m_PosX+m_VarX2+2 && X<m_PosX+m_Width-2 && Y>=m_PosY+m_VarY0 && Y<m_ScrollY0 )
        {
            if( m_FirstLine>0 )
            {
                --m_FirstLine;
        ...
        else if( _Pressed && _Button==TW_MOUSE_LEFT && X>=m_PosX+m_VarX2+2 && X<m_PosX+m_Width-2 && Y>=m_ScrollY1 && Y<m_PosY+m_VarY1 )
        {
        ...
        else if( _Pressed && !m_MouseDrag && m_Resizable && !m_IsPopupList && _Button==TW_MOUSE_LEFT && X>=m_PosX && X<m_PosX+m_Font->m_CharHeight && Y>=m_PosY && Y<m_PosY+m_Font->m_CharHeight )
        {
            m_MouseDrag = true;
        ...
        else if( _Pressed && !m_MouseDrag && m_Resizable && !m_IsPopupList && _Button==TW_MOUSE_LEFT && X>=m_PosX+m_Width-m_Font->m_CharHeight && X<m_PosX+m_Width && Y>=m_PosY && Y<m_PosY+m_Font->m_CharHeight )
        {
            m_MouseDrag = true;
        ...
        else if( _Pressed && !m_MouseDrag && m_Resizable && !m_IsPopupList && _Button==TW_MOUSE_LEFT && X>=m_PosX && X<m_PosX+m_Font->m_CharHeight && Y>=m_PosY+m_Height-m_Font->m_CharHeight && Y<m_PosY+m_Height )
        {
            m_MouseDrag = true;
        ...
        else if( _Pressed && !m_MouseDrag && m_Resizable && !m_IsPopupList && _Button==TW_MOUSE_LEFT && X>=m_PosX+m_Width-m_Font->m_CharHeight && X<m_PosX+m_Width && Y>=m_PosY+m_Height-m_Font->m_CharHeight && Y<m_PosY+m_Height )
        {
            m_MouseDrag = true;
        ...
        else if( _Pressed && g_TwMgr->m_FontResizable && !m_IsPopupList && X>=m_PosX+m_Font->m_CharHeight && X<m_PosX+2*m_Font->m_CharHeight && Y>m_PosY && Y<m_PosY+m_Font->m_CharHeight )
        {
            // change font
            if( _Button==TW_MOUSE_LEFT )
            {
        ...
        else if( _Pressed && m_Iconifiable && !m_IsPopupList && _Button==TW_MOUSE_LEFT && X>=m_PosX+m_Width-2*m_Font->m_CharHeight && X<m_PosX+m_Width-m_Font->m_CharHeight && Y>m_PosY && Y<m_PosY+m_Font->m_CharHeight )
        {
            // minimize
            g_TwMgr->Minimize(this);
            ANT_SET_CURSOR(Arrow);
        }
        else if( m_IsHelpBar && _Pressed && !g_TwMgr->m_IsRepeatingMousePressed && X>=m_PosX+m_VarX0 && X<m_PosX+m_Width-m_Font->m_CharHeight && Y>m_PosY+m_Height-m_Font->m_CharHeight && Y<m_PosY+m_Height )
        {
            /*
            ...
            for( CustomMap::iterator it=m_CustomRecords.begin(); it!=m_CustomRecords.end(); ++it )
            {
                ...
                if( (s==1 || sProxy->m_CustomCaptureFocus) && !CustomHandled && sProxy!=NULL && sProxy->m_CustomMouseButtonCallback!=NULL && r.m_XMin<r.m_XMax && r.m_Y0<r.m_Y1 && r.m_YMin<=r.m_YMax && r.m_YMin>=r.m_Y0 && r.m_YMax<=r.m_Y1 )
                {
                    if( sProxy->m_CustomCaptureFocus || (X>=r.m_XMin && X<r.m_XMax && Y>=r.m_YMin && Y<r.m_YMax) )
                    {
                        ...
                        CustomHandled = sProxy->m_CustomMouseButtonCallback(_Button, _Pressed, X-r.m_XMin, Y-r.m_Y0, r.m_XMax-r.m_XMin, r.m_Y1-r.m_Y0, sProxy->m_StructExtData, sProxy->m_StructClientData, this, r.m_Var);
                        s = 2; // force s-loop exit
                    }
                }
                else if( sProxy!=NULL )
...
void DrawArc(int X, int Y, int _Radius, float _StartAngleDeg, float _EndAngleDeg, color32 _Color) // angles in degree
{
    ITwGraph *Gr = g_TwMgr->m_Graph;
    ...
    int x0 = int(X + _Radius * cosf(startAngle) + 0.5f);
    int y0 = int(Y - _Radius * sinf(startAngle) + 0.5f);
    int x1, y1;
    float angle = startAngle+stepAngle;

    for( int i=0; i<numSteps; ++i, angle+=stepAngle )
    {
        x1 = int(X + _Radius * cosf(angle) + 0.5f);
        y1 = int(Y - _Radius * sinf(angle) + 0.5f);
    ...
    if( fullCircle )
    {
        x1 = int(X + _Radius * cosf(startAngle) + 0.5f);
        y1 = int(Y - _Radius * sinf(startAngle) + 0.5f);
    }
    else
    {
        x1 = int(X + _Radius * cosf(endAngle) + 0.5f);
        y1 = int(Y - _Radius * sinf(endAngle) + 0.5f);
    }
    Gr->DrawLine(x0, y0, x1, y1, _Color, true);
}
...
void CTwBar::RotoOnMouseMove(int X, int Y)
{
    CPoint p(X, Y);
    if( m_Roto.m_Active )
    {
...
void CTwBar::RotoOnLButtonDown(int X, int Y)
{
    CPoint p(X, Y);
...
void CTwBar::RotoOnLButtonUp(int /*X*/, int /*Y*/)
{
...
void CTwBar::RotoOnMButtonDown(int X, int Y)
{
    if( !m_Roto.m_Active )
    {
...
void CTwBar::RotoOnMButtonUp(int X, int Y)
{
    if( m_Roto.m_ActiveMiddle )
    {
...
void CTwBar::EditInPlaceStart(CTwVarAtom* Var, int X, int Y, int Width)
{
    if( m_EditInPlace.m_Active )
        EditInPlaceEnd(true);

    m_EditInPlace.m_Active = true;
    m_EditInPlace.m_Var = Var;
    m_EditInPlace.m_X = X;
    m_EditInPlace.m_Y = Y;
    m_EditInPlace.m_Width = Width;
...
bool CTwBar::EditInPlaceMouseMove(int X, int Y, bool _Select)
{
    if ( !m_EditInPlace.m_Active || Y<m_PosY+m_EditInPlace.m_Y || Y>m_PosY+m_EditInPlace.m_Y+m_Font->m_CharHeight )
        return false;
