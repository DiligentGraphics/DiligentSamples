...
struct CTwMgr
{
    ...
    //void              SetProcessing(bool processing)  { m_Processing = processing; }
                        ...
                        CTwMgr(ETwGraphAPI _GraphAPI, void *_Device, void *_ImmediateContext, int _WndID);
                        ~CTwMgr();
...
{ 
    ...
    CRect(int iX, int iY, int iW, int iH) : X(iX), Y(iY), W(iW), H(iH) {}
