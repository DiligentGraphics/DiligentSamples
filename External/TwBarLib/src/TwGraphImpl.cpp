...
int CTwGraphImpl::Init(int BackBufferFormat, int DepthStencilFormat)
{
    ...
    try
    {
        ...
        ShaderCI.Source = bIsDX ? g_LineRectVS_DX : g_LineRectVS_GL;
        ShaderCI.Desc.ShaderType = SHADER_TYPE_VERTEX;
        ShaderCI.Desc.Name = "AntTwBar: LineRectVS";
        ShaderCI.UseCombinedTextureSamplers = true;
        m_pDev->CreateShader( ShaderCI, &m_pLineRectVS );

        ShaderCI.Source = bIsDX ? g_LineRectCstColorVS_DX : g_LineRectCstColorVS_GL;
        ShaderCI.Desc.ShaderType = SHADER_TYPE_VERTEX;
        ShaderCI.Desc.Name = "AntTwBar: LineRectCstColorVS";
        m_pDev->CreateShader( ShaderCI, &m_pLineRectCstColorVS );

        ShaderCI.Source = bIsDX ? g_LineRectPS_DX : g_LineRectPS_GL;
        ShaderCI.Desc.ShaderType = SHADER_TYPE_PIXEL;
        ShaderCI.Desc.Name = "AntTwBar: LineRectPS";
        m_pDev->CreateShader( ShaderCI, &m_pLineRectPS );

        ShaderCI.Source = bIsDX ? g_TextVS_DX : g_TextVS_GL;
        ShaderCI.Desc.ShaderType = SHADER_TYPE_VERTEX;
        ShaderCI.Desc.Name = "AntTwBar: TexVS";
        m_pDev->CreateShader( ShaderCI, &m_pTextVS );

        ShaderCI.Source = bIsDX ? g_TextCstColorVS_DX : g_TextCstColorVS_GL;
        ShaderCI.Desc.ShaderType = SHADER_TYPE_VERTEX;
        ShaderCI.Desc.Name = "AntTwBar: TextCstColorVS";
        m_pDev->CreateShader( ShaderCI, &m_pTextCstColorVS );

        ShaderCI.Source = bIsDX ? g_TextPS_DX : g_TextPS_GL;
        ShaderCI.Desc.ShaderType = SHADER_TYPE_PIXEL;
        ShaderCI.Desc.Name = "AntTwBar: TextPS";
        m_pDev->CreateShader( ShaderCI, &m_pTextPS );
    }
    catch( const std::runtime_error& )
    {
    ...
    ShaderResourceVariableDesc Vars[] = {ShaderResourceVariableDesc{SHADER_TYPE_PIXEL, "g_Font", SHADER_RESOURCE_VARIABLE_TYPE_MUTABLE}};
    ...
    m_pPSO[static_cast<int>(PSO_ID::Text)]->BindStaticResources(SHADER_TYPE_VERTEX | SHADER_TYPE_PIXEL, m_pResourceMapping, BIND_SHADER_RESOURCES_VERIFY_ALL_RESOLVED );
    ...
    m_pPSO[static_cast<int>(PSO_ID::TextCstColor)]->BindStaticResources(SHADER_TYPE_VERTEX | SHADER_TYPE_PIXEL, m_pResourceMapping, BIND_SHADER_RESOURCES_VERIFY_ALL_RESOLVED );
    ...
    m_pPSO[static_cast<int>(PSO_ID::Line)]->BindStaticResources(SHADER_TYPE_VERTEX | SHADER_TYPE_PIXEL, m_pResourceMapping, BIND_SHADER_RESOURCES_VERIFY_ALL_RESOLVED );
    ...
    m_pPSO[static_cast<int>(PSO_ID::LineAA)]->BindStaticResources(SHADER_TYPE_VERTEX | SHADER_TYPE_PIXEL, m_pResourceMapping, BIND_SHADER_RESOURCES_VERIFY_ALL_RESOLVED );
    ...
    m_pPSO[static_cast<int>(PSO_ID::Triangle_CstColor)]->BindStaticResources(SHADER_TYPE_VERTEX | SHADER_TYPE_PIXEL, m_pResourceMapping, BIND_SHADER_RESOURCES_VERIFY_ALL_RESOLVED );
    ...
    m_pPSO[static_cast<int>(PSO_ID::Triangle_Multisample)]->BindStaticResources(SHADER_TYPE_VERTEX | SHADER_TYPE_PIXEL, m_pResourceMapping, BIND_SHADER_RESOURCES_VERIFY_ALL_RESOLVED );
    ...
    m_pPSO[static_cast<int>(PSO_ID::Triangle_CullNone)]->BindStaticResources(SHADER_TYPE_VERTEX | SHADER_TYPE_PIXEL, m_pResourceMapping, BIND_SHADER_RESOURCES_VERIFY_ALL_RESOLVED );
    ...
    m_pPSO[static_cast<int>(PSO_ID::Triangle_CullCW)]->BindStaticResources(SHADER_TYPE_VERTEX | SHADER_TYPE_PIXEL, m_pResourceMapping, BIND_SHADER_RESOURCES_VERIFY_ALL_RESOLVED );
    ...
    m_pPSO[static_cast<int>(PSO_ID::Triangle_CullCCW)]->BindStaticResources(SHADER_TYPE_VERTEX | SHADER_TYPE_PIXEL, m_pResourceMapping, BIND_SHADER_RESOURCES_VERIFY_ALL_RESOLVED );
...
void CTwGraphImpl::DrawLine(int _X0, int _Y0, int _X1, int _Y1, color32 _Color0, color32 _Color1, bool _AntiAliased)
{
    ...
    //if( mappedVertices )
    {
        ...
        m_pDevImmContext->UpdateBuffer(m_pLineVertexBuffer, 0, sizeof(vertices), vertices, RESOURCE_STATE_TRANSITION_MODE_TRANSITION);
...
void CTwGraphImpl::DrawRect(int _X0, int _Y0, int _X1, int _Y1, color32 _Color00, color32 _Color10, color32 _Color01, color32 _Color11)
{
    ...
    //if( mappedVertices )
    {
        ...
        //mappedVertices.Unmap();
        m_pDevImmContext->UpdateBuffer(m_pRectVertexBuffer, 0, sizeof(vertices), vertices, RESOURCE_STATE_TRANSITION_MODE_TRANSITION);
...
void CTwGraphImpl::BuildText(void *_TextObj, const std::string *_TextLines, color32 *_LineColors, color32 *_LineBgColors, int _NbLines, const CTexFont *_Font, int _Sep, int _BgWidth)
{
    ...
    if( textVerts!=NULL )
    {
        ...
        m_pDevImmContext->UpdateBuffer(textObj->m_pTextVertexBuffer, 0, (Uint32)m_textVtxBuffer.size() * sizeof(m_textVtxBuffer[0]), textVerts, RESOURCE_STATE_TRANSITION_MODE_TRANSITION);
    }
    if( bgVerts!=NULL )
    {
        //textObj->m_pBgVertexBuffer->Unmap(m_pDevImmContext);
        m_pDevImmContext->UpdateBuffer(textObj->m_pBgVertexBuffer, 0, (Uint32)m_mappedVertices.size() * sizeof(m_mappedVertices[0]), bgVerts, RESOURCE_STATE_TRANSITION_MODE_TRANSITION);
    }
}
...
void CTwGraphImpl::DrawTriangles(int _NumTriangles, int *_Vertices, color32 *_Colors, Cull _CullMode)
{
    ...
    if( mappedVertices )
    {
        ...
        m_pDevImmContext->UpdateBuffer(m_pTrianglesVertexBuffer, 0, 3*_NumTriangles*sizeof(CLineRectVtx), mappedVertices, RESOURCE_STATE_TRANSITION_MODE_TRANSITION);
        //mappedVertices.Unmap();
        ...
        pLineRectPSO->BindStaticResources(SHADER_TYPE_VERTEX | SHADER_TYPE_PIXEL, m_pResourceMapping, BIND_SHADER_RESOURCES_KEEP_EXISTING | BIND_SHADER_RESOURCES_VERIFY_ALL_RESOLVED);
