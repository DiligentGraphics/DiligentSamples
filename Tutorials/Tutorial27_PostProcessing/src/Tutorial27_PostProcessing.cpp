...
struct Tutorial27_PostProcessing::ShaderSettings
{
    ...
    {
        VERIFY_EXPR(!SRVariants.empty() && ActiveUpscalerIdx < static_cast<Int32>(SRVariants.size()));
        return SRVariants[ActiveUpscalerIdx];
    }

    bool IsSREnabled() const
    {
        return SREnabled && !SRVariants.empty();
    }

    bool IsTemporalUpscaling() const
    {
        return IsSREnabled() && GetActiveUpscalerInfo().Type == SUPER_RESOLUTION_TYPE_TEMPORAL;
    }

    bool IsSpatialUpscaling() const
    {
        return IsSREnabled() && !IsTemporalUpscaling();
    }
    ...
    bool SupportsSharpness() const
    {
        const auto& Info = GetActiveUpscalerInfo();
...
void Tutorial27_PostProcessing::Initialize(const SampleInitInfo& InitInfo)
{
    ...
    if (m_pSRFactory)
    {
        ...
        m_pSRFactory->EnumerateVariants(NumVariants, nullptr);
        m_ShaderSettings->SRVariants.resize(NumVariants);
        m_pSRFactory->EnumerateVariants(NumVariants, m_ShaderSettings->SRVariants.data());
    }
}
...
void Tutorial27_PostProcessing::Render()
{
    ...
    ITextureView* pHDRColorSRV = m_Resources[RESOURCE_IDENTIFIER_RADIANCE0 + CurrFrameIdx].GetTextureSRV();

    if (m_ShaderSettings->IsTemporalUpscaling())
    {
        ComputeTemporalUpscaling();
        pHDRColorSRV = m_Resources[RESOURCE_IDENTIFIER_UPSCALING].GetTextureSRV();
    }
    else if (m_ShaderSettings->TAAEnabled)
    {
        ComputeTAA();
        pHDRColorSRV = m_TemporalAntiAliasing->GetAccumulatedFrameSRV();
    }

    UpdateSSRSourceColor(pHDRColorSRV);

    if (m_ShaderSettings->BloomEnabled)
    {
        ComputeBloom(pHDRColorSRV);
        pHDRColorSRV = m_Bloom->GetBloomTextureSRV();
    }

    ComputeToneMapping(pHDRColorSRV);

    ITextureView* pFinalColorSRV = m_Resources[RESOURCE_IDENTIFIER_TONE_MAPPING].GetTextureSRV();
    if (m_ShaderSettings->IsSpatialUpscaling())
    {
        ComputeSpatialUpscaling();
        pFinalColorSRV = m_Resources[RESOURCE_IDENTIFIER_UPSCALING].GetTextureSRV();
    }

    ComputeGammaCorrection(pFinalColorSRV);
}
...
void Tutorial27_PostProcessing::Update(double CurrTime, double ElapsedTime, bool DoUpdateUI)
{
    ...
    SRSettings.OptimalInputWidth  = SCDesc.Width;
    SRSettings.OptimalInputHeight = SCDesc.Height;
    if (m_ShaderSettings->IsSREnabled())
    {
        SuperResolutionSourceSettingsAttribs SRAttribs;
        SRAttribs.VariantId        = m_ShaderSettings->GetActiveVariantId();
        SRAttribs.OutputWidth      = SCDesc.Width;
        SRAttribs.OutputHeight     = SCDesc.Height;
        SRAttribs.OutputFormat     = m_ShaderSettings->IsTemporalUpscaling() ? TEX_FORMAT_R11G11B10_FLOAT : TEX_FORMAT_RGBA8_UNORM_SRGB;
        SRAttribs.Flags            = m_ShaderSettings->IsTemporalUpscaling() ? SUPER_RESOLUTION_FLAG_AUTO_EXPOSURE : SUPER_RESOLUTION_FLAG_NONE;
        SRAttribs.OptimizationType = m_ShaderSettings->SROptimizationType;
        m_pSRFactory->GetSourceSettings(SRAttribs, SRSettings);
    }
    ...
    {
        m_pSRUpscaler->GetJitterOffset(m_CurrentFrameNumber, Jitter.x, Jitter.y);
...
void Tutorial27_PostProcessing::PrepareResources()
{
        ...
        m_PostFXFrameDesc.Height != m_Resources[RESOURCE_IDENTIFIER_RADIANCE0].AsTexture()->GetDesc().Height)
    {
        ...
        {
            TextureDesc Desc;
            Desc.Name      = "Tutorial27_PostProcessing::SSRSourceColor";
            Desc.Type      = RESOURCE_DIM_TEX_2D;
            Desc.Width     = m_PostFXFrameDesc.Width;
            Desc.Height    = m_PostFXFrameDesc.Height;
            Desc.Format    = TEX_FORMAT_R11G11B10_FLOAT;
            Desc.BindFlags = BIND_SHADER_RESOURCE | BIND_RENDER_TARGET;
            m_Resources.Insert(RESOURCE_IDENTIFIER_SSR_SOURCE_COLOR, Device.CreateTexture(Desc));

            float4        Color = float4(0.0, 0.0, 0.0, 1.0);
            ITextureView* pRTV  = m_Resources[RESOURCE_IDENTIFIER_SSR_SOURCE_COLOR].GetTextureRTV();
            m_pImmediateContext->SetRenderTargets(1, &pRTV, nullptr, RESOURCE_STATE_TRANSITION_MODE_TRANSITION);
            m_pImmediateContext->ClearRenderTarget(pRTV, Color.Data(), RESOURCE_STATE_TRANSITION_MODE_TRANSITION);
            m_pImmediateContext->SetRenderTargets(0, nullptr, nullptr, RESOURCE_STATE_TRANSITION_MODE_NONE);
        }
    ...
    // Resize G-Buffer when rendering resolution changes
    ...
    if (m_ShaderSettings->IsSREnabled())
    {
        ...
        if (NeedRecreate && VariantId != INTERFACE_ID{})
        {
            ...
            UpscalerDesc.Flags        = m_ShaderSettings->IsTemporalUpscaling() ? SUPER_RESOLUTION_FLAG_AUTO_EXPOSURE : SUPER_RESOLUTION_FLAG_NONE;

            if (m_ShaderSettings->SupportsSharpness())
                UpscalerDesc.Flags = UpscalerDesc.Flags | SUPER_RESOLUTION_FLAG_ENABLE_SHARPENING;
            ...
            if (m_pSRFactory)
                m_pSRFactory->CreateSuperResolution(UpscalerDesc, &m_pSRUpscaler);
...
void Tutorial27_PostProcessing::ComputeSSR()
{
    if (m_ShaderSettings->SSRStrength > 0.0)
    {
        ...
        SSRRenderAttribs.pColorBufferSRV    = m_Resources[RESOURCE_IDENTIFIER_SSR_SOURCE_COLOR].GetTextureSRV();
...
void Tutorial27_PostProcessing::ComputeBloom(ITextureView* pColorBufferSRV)
{
    ...
    BloomRenderAttribs.pColorBufferSRV = pColorBufferSRV;
    m_Bloom->Execute(BloomRenderAttribs);
}

void Tutorial27_PostProcessing::ComputeToneMapping(ITextureView* pHDRTextureSRV)
{
    ...
    ShaderResourceVariableX{RenderTech.SRB, SHADER_TYPE_PIXEL, "g_TextureHDR"}.Set(pHDRTextureSRV);
...
void Tutorial27_PostProcessing::ComputeSpatialUpscaling()
{
    ...
    SRAttribs.pOutputTextureView = m_Resources[RESOURCE_IDENTIFIER_UPSCALING].GetTextureRTV();
    SRAttribs.Sharpness          = m_ShaderSettings->Sharpness;

    ScopedDebugGroup DebugGroup{m_pImmediateContext, "SpatialUpscaling"};
    SRAttribs.pContext = m_pImmediateContext;
    m_pSRUpscaler->Execute(SRAttribs);
}
...
void Tutorial27_PostProcessing::ComputeTemporalUpscaling()
{
    ...
    float2 Jitter = {};
    m_pSRUpscaler->GetJitterOffset(m_PostFXFrameDesc.Index, Jitter.x, Jitter.y);
    ...
    SRAttribs.pOutputTextureView = m_Resources[RESOURCE_IDENTIFIER_UPSCALING].GetTextureUAV();
...
void Tutorial27_PostProcessing::UpdateSSRSourceColor(ITextureView* pSourceColorSRV)
{
    PostFXContext::TextureOperationAttribs CopyAttribs;
    CopyAttribs.pDevice        = m_pDevice;
    CopyAttribs.pDeviceContext = m_pImmediateContext;
    m_PostFXContext->CopyTextureColor(CopyAttribs, pSourceColorSRV, m_Resources[RESOURCE_IDENTIFIER_SSR_SOURCE_COLOR].GetTextureRTV());
}

void Tutorial27_PostProcessing::ComputeGammaCorrection(ITextureView* pFinalColorSRV)
{
    ...
    if (ConvertOutputToGamma)
    {
        ...
        ShaderResourceVariableX{RenderTech.SRB, SHADER_TYPE_PIXEL, "g_TextureColor"}.Set(pFinalColorSRV);
    ...
    else
    {
        ...
        m_PostFXContext->CopyTextureColor(CopyTextureAttribs, pFinalColorSRV, pRTV);
    }
}
...
void Tutorial27_PostProcessing::UpdateUI()
{
    ...
    if (ImGui::Begin("Settings", nullptr, ImGuiWindowFlags_AlwaysUseWindowPadding | ImGuiWindowFlags_AlwaysAutoResize))
    {
        ...
        if (ImGui::TreeNode("Rendering"))
        {
            ...
            if (!m_ShaderSettings->SRVariants.empty())
            {
                if (ImGui::Checkbox("Enable Super Resolution", &m_ShaderSettings->SREnabled))
                {
                    if (m_ShaderSettings->IsTemporalUpscaling())
                        m_ShaderSettings->TAAEnabled = false;
                }
            }
            ImGui::TreePop();
        }
        ...
        if (ImGui::TreeNode("Post Processing"))
        {
            ...
            if (m_ShaderSettings->IsSREnabled() && ImGui::TreeNode("Upsampling"))
            {
                const auto& SRVariants = m_ShaderSettings->SRVariants;
                if (!SRVariants.empty())
                {
                    std::vector<const char*> Names;
                    for (size_t Idx = 0; Idx < SRVariants.size(); ++Idx)
                        Names.push_back(SRVariants[Idx].Name);
                    ...
                    if (m_ShaderSettings->SupportsSharpness())
                        ImGui::SliderFloat("Sharpness", &m_ShaderSettings->Sharpness, 0.0f, 1.0f);
