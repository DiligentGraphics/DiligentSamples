...
void MengerSpongeSample::Initialize(IEngineFactory* pEngineFactory, IRenderDevice *pDevice, IDeviceContext **ppContexts, Uint32 NumDeferredCtx, ISwapChain *pSwapChain)
{
    ...
    m_pRenderScript = CreateRenderScriptFromFile( "MengerSponge.lua", m_pDevice, m_pImmediateContext, [&]( ScriptParser *pScriptParser )
    {
        ...
        auto DepthBufferFmt = m_pDevice->GetTextureFormatInfo(m_pSwapChain->GetDesc().DepthBufferFormat).Name;
                       ...
                       const float3& center, bool aoEnabled, const bool aoEdges[12], const unsigned int faceColors[6])
{
    ...
    if (level == levelMax)
    {
        ...
        if (aoRatio > 0.4999f)
            aoRatio = 0.4999f;
        float4x4 xform = float4x4::Scale( scale, scale, scale ) * float4x4::Translation( center );
...
void MengerSpongeSample::Render()
{
    ...
    m_pImmediateContext->ClearRenderTarget(nullptr, m_BackgroundColor, RESOURCE_STATE_TRANSITION_MODE_TRANSITION);
    ...
    float4x4 proj = float4x4::Projection(PI_F/4.f, aspectRatio, 0.1f, 100.0f, DeviceCaps.IsGLDevice());
    float dist = m_CamDistance + 0.4f;
    float3 camPosInv ( dist * 0.3f, dist * 0.0f, dist * 2.0f );
    float4x4 view = float4x4::Translation(camPosInv);
