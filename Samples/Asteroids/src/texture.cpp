...
void InitializeTexture2D(
    ...
    ID3D12Resource* texture, const D3D12_RESOURCE_DESC* desc,
    UINT blockWidth, UINT blockHeight, UINT bytesPerPixel,
    ...
    D3D12_RESOURCE_STATES stateAfter)
{
    ...
    for (UINT a = 0; a < arraySize; ++a) {
        for (UINT m = 0; m < mipLevels; ++m) {
            ...
            placed.Footprint.RowPitch = AlignUp<UINT>(placed.Footprint.Width / blockWidth * bytesPerPixel, D3D12_TEXTURE_DATA_PITCH_ALIGNMENT);
            placed.Offset = totalSize;
        
            totalSize = AlignUp<UINT64>(placed.Offset + (size_t{placed.Footprint.RowPitch} * size_t{placed.Footprint.Height} / blockHeight),
    ...
    for (UINT a = 0; a < arraySize; ++a) {
        for (UINT m = 0; m < mipLevels; ++m) {
            ...
            for (UINT y = 0; y < height_mip / blockHeight; ++y) {
                memcpy(dataDst + y*size_t{rowPitchDst}, dataSrc + y*size_t{rowPitchSrc}, size_t{bytesPerPixel} * width_mip / blockWidth);
            }
        }
    }
    ...
    D3D12_RESOURCE_STATES stateAfter )
{
    ...
    for (UINT a = 0; a < arraySize; ++a) {
        for (UINT m = 0; m < desc.MipLevels; ++m) {
            ...
            auto rowPitch = PixelSize * width / BlockWidth;
            auto bytes = rowPitch * height / BlockHeight;
    ...
    InitializeTexture2D(device, cmdQueue, *texture, &desc, BlockWidth, BlockHeight, PixelSize, initialData.data(), stateAfter);
